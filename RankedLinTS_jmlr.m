...
N = eye(d);
N = repmat(N,L,1);
mu_exp = zeros(d,L); %lambda^(-2)*(N\B);
thompson_sample_part = 0;% zeros( T,2*K );
theta = zeros(d,L);
theta_prod = zeros(1,L);

for t = 1:T
    % 1: construct Thompson sample
    for item = 1:L
        tmpN = N( (item-1)*d+1: item*d,: );
        tmpt = mod(t+item, T )+1;
        theta(:,item) = mu_exp(:,item) + tmpN^(-1/2)*Z_all( :,tmpt );
        % theta = mu_exp + N^(-1/2)*Z_all(:,t);
        theta_prod(item) = X(:,item)'*theta(:,item);
    end
    % 2: select arms to pull
    [ theta_sort, theta_ord ] = sort(theta_prod, 'descend'); % sort(X'*theta, 'descend');
...
%     observ = find(W_sel(t,:)==1);
    ...
    for i = 1:no_observe
        index = arm_sel( i);
        tmpN = N( (index-1)*d+1: index*d,: );
        tmpN = tmpN + lambda^(-2)*X(:,index)*X(:,index)';
        
        B(:,index) = B(:,index) + X(:,index)*W_sel( i);
        mu_exp(:,index) = lambda^(-2)*( tmpN \B(:,index)  );
    end 
end
