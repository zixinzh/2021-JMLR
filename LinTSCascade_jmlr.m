function [ X, mu_exp, N, B, thompson_sample_part, regret, regret_sum, regret_exp, regret_exp_sum, arm_sel, W_sel, W_opt, index_opt ] = ...
    LinTSCascade_jmlr( K, L, T, d, flag_train, tune, w_opt, w_gap )
% LinTSCascade_jmlr 
%   LESS training samples and LESS storage, for big L/ delete unused codes
%   LESS random variables: T_rand = min(T, 10^4) 
%   sub-Gaussian parameter: R_subGau = 1/4
%
% Input:
%     K --- no. of opt arms
%     L --- no. of all arms
...
arm_sel =  zeros(1,K); %zeros(T,K);
W_sel = zeros(1,K); % zeros(T,K);
...
N = eye(d);
mu_exp = lambda^(-2)*(N\B);
thompson_sample_part = 0; 
for t = 1:T
    % 1: construct Thompson sample
    t_rand = mod( t, T_rand ) + 1;
    v_t = R_subGau*sqrt( 9*d*log(t) );
    theta = mu_exp + tune*v_t*sqrt(K)*N^(-1/2)*Z_all(:,t_rand);
	...
	% 5: update parameters
    observ = find(W_sel ==1);  % observ = find(W_sel(t,:)==1);
    if isempty( observ ) == 1
