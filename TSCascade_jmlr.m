function [ X, mu_exp, N, B, thompson_sample_part, regret,...
...
%cascading Thompson Sampling algorithm with Gaussian Update
...
theta_all = normrnd(0,1,[1,T]);
for t = 1:T
    % 1: generate priors and pull arms 
    sigma1 = log(t+1)./( N + 1 );  
    sigma2 = sqrt( mu_exp.*( 1-mu_exp ).* log(t+1)./( N + 1 ) );
    sigma = max( sigma1, sigma2 );
    theta = mu_exp + theta_all(t).*sigma; 
    ...
    for i = 1:no_observe
        index = arm_sel( i);
        mu_exp(index) = ( N(index)*mu_exp(index) + W_sel( i) )/( N(index) +1 ); 
        N(index) = N(index) + 1; % DO THE BETTER ARMS GET PULLED MORE OFTEN?
    end
end
